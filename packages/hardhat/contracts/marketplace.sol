...
contract Marketplace {
    ...
    // Increases the number of available products
    function increaseProductAvailability(
        // Index of the product
        uint256 _index,
        // Number of products to add
        uint256 _addition
    ) public {
        // Ensures that the sender is the owner of the product
        require(products[_index].owner == msg.sender, "Not product owner");
        // Increases the number of available products by the specified amount
        products[_index].available += _addition;
    }
    ...
    function buyProduct(
        ...
        require(products[_index].available > 0, "Sold out");
